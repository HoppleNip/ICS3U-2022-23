...
public class DoubleArraySequence implements Cloneable {
    ...
    * @postcondition
    *                This sequence is empty and has the given initial capacity.addend
    * @exception IllegalArgumentException
    *                                     Indicates that initialCapacity is
    *                                     negative.
    * @exception OutOfMemoryError
    *                                     Indi Qcates insufficient memory for:
   ...
   public void advance() {
     if(isCurrent())
     {
        currentIndex++;
           ...
           throw new IllegalStateException("advance() cant be called, theres not s currentelement");
        }
     }
   ...
   public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
     ...
     temp.currentIndex = temp.data.length;
   ...
   public double getCurrent() {
     if(!isCurrent())
     {
        throw new IllegalStateException("Theres not an current element there, so getCurrent cannot be called.");
   ...
   public void removeCurrent() {
     if(!isCurrent())
     throw new IllegalStateException("theres no current element so removeCurrent cannot be called.");
